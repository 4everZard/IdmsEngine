...
import ca.on.moh.idms.persister.EngineParameterPersister;
import ca.on.moh.idms.persister.EngineParameterPersisterImpl;
import ca.on.moh.idms.vo.EngineParameter;


public class EngineParameterManager  {

	static final long serialVersionUID = 162504;
	private EngineParameterPersister persister = new EngineParameterPersisterImpl();
	...
	public ValueObject findByPK(ValueObject vo)throws Exception {
		EngineParameter obj = (EngineParameter)vo;
		return  persister.findByPK(obj.getParameterId());
	}
	...
	public List <ValueObject> findByObject(ValueObject vo)throws Exception {
		...
		EngineParameter obj = (EngineParameter)vo;
		String where = "where";
		if(obj.getParameterId() != Integer.MIN_VALUE && obj.getParameterId() != 0){ 
			where = where + " PARAMETER_ID=" + obj.getParameterId() + " AND"; 
		}
		if(obj.getParameterValue() != null && !obj.getParameterValue().trim().equals("")){ 
			where = where + " UPPER(PARAMETER_VALUE) like UPPER('%" + obj.getParameterValue() + "%') AND"; 
		}
		if(obj.getEffectiveDate() != null){ 
			SimpleDateFormat format = new SimpleDateFormat(ValueObject.SIMPLE_DATE_FORMAT);
			String aString = format.format( new java.sql.Timestamp(obj.getEffectiveDate().getTime()));
			where = where + " EFFECTIVE_DATE = to_date('" + aString + "','" + ValueObject.SQL_DATE_FORMAT + "') AND"; 
		}
		if(obj.getEndDate() != null && !obj.getEndDate().trim().equals("")){ 
			where = where + " UPPER(END_DATE) like UPPER('%" + obj.getEndDate() + "%') AND"; 
		}
		if(obj.getParameterCreatedBy() != Integer.MIN_VALUE && obj.getParameterCreatedBy() != 0){ 
			where = where + " PARAMETER_CREATED_BY=" + obj.getParameterCreatedBy() + " AND"; 
		}
		if(obj.getParameterUpdatedBy() != Integer.MIN_VALUE && obj.getParameterUpdatedBy() != 0){ 
			where = where + " PARAMETER_UPDATED_BY=" + obj.getParameterUpdatedBy() + " AND"; 
		}
		if(obj.getParameterName() != null && !obj.getParameterName().trim().equals("")){ 
			where = where + " UPPER(PARAMETER_NAME) like UPPER('%" + obj.getParameterName() + "%') AND"; 
		}
	...
	public void executeDelete(ValueObject vo) throws Exception {
		EngineParameter obj = (EngineParameter)vo;
		persister.deleteByPK(obj.getParameterId());
	}
