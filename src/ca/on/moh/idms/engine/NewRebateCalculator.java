...
public class NewRebateCalculator {
	...
	public static void main (String [] args) throws Exception {
		...
		NewRebateCalculator calculator = new NewRebateCalculator();
	...
	public List<RebateVO> step1CreateTemp01(String manufacturerCode, Connection conn) throws Exception{// here din is "02418401" for the example
		...
		String dateSql = "select * from CALC_PERIOD where PERIOD_CODE = '2015Q2'";
		
		
		//String calcStartDate = ConfigFromDB.getConfigPropertyFromDB(RebateConstant.HISTORY_START_DATE);	//JUN 29 2015
		//String calcEndDate = ConfigFromDB.getConfigPropertyFromDB(RebateConstant.HISTORY_END_DATE);	//JUN 30 2015
		...
		ResultSet rs = null;
		try{
			String calcStartDate = null;
			String calcEndDate = null;
			
			ps = conn.prepareStatement(dateSql);
			rs = ps.executeQuery();
			while (rs.next()){
				calcStartDate = rs.getDate("START_DATE").toString();
				calcEndDate = rs.getDate("END_DATE").toString();
			}
			String sql = "select DIN_PIN,DT_OF_SERV, ADJUDICATION_DT, PROF_FEE_ALLD,QTY,DRG_CST_ALLD,PROG_ID,PROD_SEL, INTERVENTION_1, " +
					...
					"from CLMHIST " +
					"where CURR_STAT = 'P' and Trunc(DT_OF_SERV) >= to_date('" + calcStartDate + "','MM-DD-YYYY') AND " +
							"Trunc(DT_OF_SERV) <= to_date('" + calcEndDate + "','MM-DD-YYYY') AND " +
	...
	public List<RebateVO> step2CreateTemp03(String manufacturerCode,  Connection conn) throws Exception{// here din is "02418401" for the example
		...
		try{
			...
			String sql = "insert into TEMP03 (DIN_PIN,DIN_DESC,FIRST_PRICE,REC_EFF_DT,REC_CREATE_TIMESTAMP,MANUFACTURER_CD) " +
					"select a.DIN_PIN,a.DIN_DESC,b.INDIVIDUAL_PRICE AS FIRST_PRICE,b.REC_EFF_DT,b.REC_CREATE_TIMESTAMP,a.MANUFACTURER_CD from DRUG a " +
					"join PRICE b on a.DRUG_ID = b.DRUG_ID and b.IS_FIRST_PRICE = 'Y'";
			ps = conn.prepareStatement(sql);
			ps.executeUpdate();
	 ...
	 * @throws Exception
	 */
	public List<RebateVO> step4CreateTemp99(String manufacturerCode,  Connection conn) throws Exception{
		...
		try{
			...
			String sql = "insert into TEMP99 (DIN_PIN,DIN_DESC,YYYY_PRICE,REC_EFF_DT,REC_CREATE_TIMESTAMP,MANUFACTURER_CD) " +
					"select a.DIN_PIN,a.DIN_DESC,b.INDIVIDUAL_PRICE AS YYYY_PRICE,b.REC_EFF_DT,b.REC_CREATE_TIMESTAMP,a.MANUFACTURER_CD from DRUG a " +
					"join PRICE b on a.DRUG_ID = b.DRUG_ID and b.IS_YY_PRICE = 'Y'";
